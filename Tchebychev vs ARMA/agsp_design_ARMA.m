...
cvx_begin quiet
    variable ia(Ka,1)
    variable ib(Kb+1,1)
        minimize( norm(NM*ib - diag(response)*MM*ia - response) )
    subject to
        max(abs(V * ia)) <= radius;
cvx_end
warning on;
...
B = fliplr(vander(mu)); 
b = lsqlin(B(:,1:Kb+1)./repmat(B(:,1:Ka+1)*a,1,Kb+1), response);
...
% -------------------------------------------------------------------------
if radius >= 1,
    [a, b] = dlsqrat(mu, response, Kb, Ka, a(2:end)); a = [1; a];
end
...
% error
error = norm(rARMA - response)./norm(mu);

if show,
    fprintf('error %f\n', error);
    figure; 
    plot(mu, rARMA, 'ro--',  mu, response, 'k');
end
